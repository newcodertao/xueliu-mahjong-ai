...
def read_discard_tiles(detections: list[Detection], confidence_threshold: float = 0.6) -> list[str]:
    filtered = [det for det in detections if det.confidence >= confidence_threshold]
    return [det.label for det in sorted(non_max_suppression(filtered), key=lambda item: (item.y1, item.x1))]
